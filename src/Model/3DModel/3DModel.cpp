...
Model3D::Model3D(std::string path){
    if(path.empty()){
        print_error("Where is the model, Pёsss\n");
        throw WRONG_PATH;
    }
...
bool Model3D::init(){
    ...
    print_debug("Class Model3D - method constructor\n");

    // load model

    Assimp::Importer importer;
    const aiScene* scene = importer.ReadFile(this->_pathModel, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);

    if(!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero
    {
        print_error("ERROR::ASSIMP:: %s\n ",importer.GetErrorString());
        return false;
    }

    this->_directory = this->_pathModel.substr(0, this->_pathModel.find_last_of('/'));
    print_debug("directory = %s\n", this->_directory.c_str());
